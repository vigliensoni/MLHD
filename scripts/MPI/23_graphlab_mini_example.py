...
# for ((i=0; i<=3;i++)); do echo `sqsub -r 5m -f xeon -q mpi --mpp 8GB -o 23_graphlab_test -n 8 /home/vigliens/python/bin/python /home/vigliens/Documents/2_CODE/4_SCRIPTS/MPI/23_graphlab_mini_example.py $i`; done
...
def storing_results(out_folder, file_no, regularization_vals, num_factors, user_side_data_par, train, test, time):
    ...
    with open(out_folder + str(file_no), 'w') as out_file:
        out_file.write(','.join([str(regularization_vals),
                                 str(num_factors),
...
# gl.set_runtime_config('GRAPHLAB_DEFAULT_NUM_PYLAMBDA_WORKERS', 2)
    ...
    storing_results(OUT_FOLDER, file_no, regularization_vals, num_factors, user_side_data_par, train, test, model.training_time)
