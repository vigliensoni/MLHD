...
# These values can be used to extract features such as how much a user sticks to the
# same songs, for example/
# ################################################################################
	...
	for file_in_tar in GVM_classes.folder_iterator(TEMP_FOLDER):
		...
		for song_data in songs_data:	# Iterate over all songs in the list
			...
			if sum(number_scrobbles_per_ranking_zone) != 0:	
				...
				out_file.write('\t'.join([str(user_features.lfid), str(no_scrobbles), ','.join(number_scrobbles_per_ranking_zone_string), '\n']))				
			out_file.close()
