...
# sqsub -r 30m -f xeon -q mpi --mpp 4GB -o .ranking_tracks_test -n 8 python 4_SCRIPTS/MPI/11_ranking_artists_album_tracks_DUP.py 0

# for ((i=0; i<=11;i++)); do echo `sqsub -r 2h -f xeon -q mpi --mpp 4GB -o .ranking_tracks_test -n 48 python 4_SCRIPTS/MPI/11_ranking_artists_album_tracks_DUP.py $i`; done
...
if __name__ == "__main__":
	...
	full_frequencies = dict() # dictionary with aggregated frequencies of listeners
	list_of_dict = [] # list of dictionaries

	files_in_TAR_file = GVM_classes.folder_iterator(TEMP_FOLDER)


	i = 0
	for file_in_tar in files_in_TAR_file[:]:
		...
		if i < 100:
			...
			# print 'i:{2}, file_in_tar:{0}, len_freq:{1}'.format(file_in_tar, len(full_frequencies.items()), i)
			i += 1
			continue
		else:
			list_of_dict.append(full_frequencies)
			i = 0
			full_frequencies = dict()
		# dn = { k: d1.get(k, 0) + d2.get(k, 0) for k in set(d1) | set(d2) }
		

		# Maybe a faster approach would be add all dictionaries into one, and then merge them.
	list_of_dict.append(full_frequencies)

	dict_this_rank = dict()
	for i, d in enumerate(list_of_dict):
		# print 'Working in rank {0} on {1} of {2} files'.format(rank, i, len(list_of_dict))
		dict_this_rank = GVM_classes.dict_merger_adder(dict_this_rank, d)

	# print 'Rank {0} finished with {1} MBIDs'.format(rank, len(dict_this_rank.keys()))
	...
	# comm.Gather(dict_this_rank, dictionary_list_gathered, root=0) #Gather(self, sendbuf, recvbuf, int root=0)

	del list_of_dict, full_frequencies
	del dict_this_rank
	gc.collect()

	# print dictionary_list_gathered
	...
	if rank == 0:
		# print 'dictionary_list_gathered:{0}'.format(dictionary_list_gathered)
		...
		for i, sublist in enumerate(dictionary_list_gathered):
		# for j, dictionary in enumerate(sublist):
			# print '{0},{1} of {2}'.format(i, j, len(dictionary_list_gathered) * len(sublist))
			# print 'length_dictionaries:{0}'.format(len(dictionary.items()))

			print 'Working in rank {0} on {1} of {2} files'.format(rank, i, len(dictionary_list_gathered))
			dict_this_round = GVM_classes.dict_merger_adder(dict_this_round, sublist)
			# print 'length_dict_this_round:{0}'.format(len(dict_this_round.items()))
		
		del dictionary_list_gathered, sublist

		################################################################
		# Merge dictionary data from this core batch with previous batch
		################################################################
		# Create file if it does not exist
		if not os.path.isfile('ranking.pickle'): 
		...
		with open('ranking.pickle', 'rb') as input_file:
