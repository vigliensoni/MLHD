...
# for ((i=0; i<=31;i++)); do echo `sqsub -r 4h -q threaded --mpp 8GB -o 24_graphlab_0_1_$i_%J -n 4 /home/vigliens/python/bin/python /home/vigliens/Documents/2_CODE/4_SCRIPTS/MPI/24_graphlab_0_1_run_just_1e-06_100factors.py $i`; done  #REDFIN
# for ((i=0; i<=31;i++)); do echo `sqsub -r 12h -f xeon -q threaded --mpp 8GB -o 24_graphlab_0_1_$i_%J -n 8 /home/vigliens/python/bin/python /home/vigliens/Documents/2_CODE/4_SCRIPTS/MPI/24_graphlab_0_1_run_just_1e-06_100factors.py $i`; done  #ORCA
...
OUT_FOLDER = '/scratch/vigliens/out/'  # ComputeCanada
...
OUT_FOLDER_PATH = OUT_FOLDER + SQ_JOBID + '/'
if not os.path.exists(OUT_FOLDER_PATH):
    os.makedirs(OUT_FOLDER_PATH)
print 'OUT_FOLDER_PATH:{0}'.format(OUT_FOLDER_PATH)
...
def model_training(observation_data,
    ...
    model = gl.factorization_recommender.create(
        ...
        solver=solver,
        sgd_trial_sample_proportion=1.00)
        # sgd_trial_sample_proportion=0.01)
    return model
...
def storing_results(out_folder, file_no, regularization_vals, num_factors, user_side_data_par, train, test, time):
    """
    """
    with open('/'.join([out_folder, str(file_no)]), 'a') as out_file:
...
if __name__ == "__main__":
    ...
    for i in range(1):  # number of created models
    ...
    # gl.set_runtime_config('GRAPHLAB_DEFAULT_NUM_PYLAMBDA_WORKERS', 2)
        ...
        storing_results(OUT_FOLDER_PATH, '-'.join([format(file_no, '03'), str(i)]), regularization_vals, num_factors, user_side_data_par, train, test, model.training_time)
