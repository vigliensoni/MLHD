...
# for ((i=0; i<=31;i++)); do echo `sqsub -r 4h -q threaded --mpp 8GB -o 29_graphlab_side_data_comparison_for_best_values_1e-04_11f_$i_%J -n 4 /home/vigliens/python/bin/python /home/vigliens/Documents/2_CODE/4_SCRIPTS/MPI/29_graphlab_side_data_comparison_for_best_values_1e-04_11f.py $i`; done
...
OUT_FOLDER = '/scratch/vigliens/out_lambda_11f/'  # ComputeCanada
...
def lambda_values():
    """
    Create sequence of 20 lambda values in log10 scale.

    [0.1, 0.316227766016838, 0.01, 0.0316227766016838, 0.001, ...,
     1e-10, 3.1622776601683795e-10]
    """
    out = []
    for i in range(10):
        r = 1 / (10 * pow(10, float(i)))
        rm = r * pow(10, 0.5)
        out.append(r)
        out.append(rm)
    return out
...
def storing_results(out_folder, file_no, regularization_vals, num_factors, user_side_data_par, train, test, time):
    """
    """
    with open(out_folder + str(file_no), 'a') as out_file:
